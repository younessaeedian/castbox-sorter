console.log("UI Renderer Module Loaded - Exact Replica");

// --- Shared Globals ---
var customListContainer = null;
var customDropdownMenu = null;
var customDropdownValue = null;
var currentSortType = "default";

// --- Constants ---
const INJECTION_POINT_SELECTOR = "div.trackListCon";
const ORIGINAL_LIST_SELECTOR = "div#trackListCon_list";

const SORT_OPTIONS = {
  default: "Default (Newest)",
  likes: "Most Likes",
  plays: "Most Plays",
  comments: "Most Comments",
  oldest: "Oldest",
};

// --- Icons for Stats (SVG) ---
// این آیکون‌ها برای نمایش تعداد لایک و پلی است (چون در لیست اصلی این عددها نیست، باید آیکون دستی بگذاریم)
// اما سعی شده دقیقاً شبیه استایل سایت باشد.
const statLikeIcon = `<img class="heart" src="data:image/png;base64,">`;
const statPlayIcon = `<svg class="heart" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<g opacity="0.7">
<path d="M3 21.2421V2.75791C2.99991 2.44869 3.07991 2.14492 3.23191 1.87727C3.38391 1.60962 3.60254 1.38758 3.86572 1.23355C4.1289 1.07952 4.42732 0.99896 4.73083 1.00001C5.03435 1.00106 5.33221 1.08368 5.59436 1.23953L21.145 10.4834C21.4052 10.6382 21.6211 10.8598 21.7711 11.126C21.9211 11.3923 22 11.6939 22 12.0009C22 12.3079 21.9211 12.6095 21.7711 12.8757C21.6211 13.142 21.4052 13.3636 21.145 13.5184L5.59436 22.7605C5.33221 22.9163 5.03435 22.9989 4.73083 23C4.42732 23.001 4.1289 22.9205 3.86572 22.7665C3.60254 22.6124 3.38391 22.3904 3.23191 22.1227C3.07991 21.8551 2.99991 21.5513 3 21.2421Z" stroke="#9B9B9B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</g>
</svg>`;
const statCommentIcon = `<svg class="heart" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg">
<g clip-path="url(#clip0_51_103)">
<g opacity="0.7">
<path d="M12 23C14.1756 23 16.3023 22.3549 18.1113 21.1462C19.9202 19.9375 21.3301 18.2195 22.1627 16.2095C22.9952 14.1995 23.2131 11.9878 22.7886 9.85401C22.3642 7.72022 21.3165 5.76021 19.7782 4.22183C18.2398 2.68345 16.2798 1.6358 14.146 1.21137C12.0122 0.78693 9.80047 1.00477 7.79048 1.83733C5.78049 2.66989 4.06253 4.07979 2.85383 5.88873C1.64514 7.69767 1 9.82441 1 12C1 13.8187 1.44 15.5334 2.22222 17.0441L1 23L6.95589 21.7778C8.46655 22.56 10.1826 23 12 23Z" stroke="#9B9B9B" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/>
</g>
</g>
<defs>
<clipPath id="clip0_51_103">
<rect width="24" height="24" fill="white"/>
</clipPath>
</defs>
</svg>`;

// --- UI Helper Functions ---
function formatDuration(seconds) {
  if (isNaN(seconds) || seconds < 1) return "00:00";
  const date = new Date(seconds * 1000);
  const hh = date.getUTCHours();
  const mm = date.getUTCMinutes();
  const ss = date.getUTCSeconds();
  if (hh) {
    return `${hh}:${String(mm).padStart(2, "0")}:${String(ss).padStart(
      2,
      "0"
    )}`;
  }
  return `${String(mm).padStart(2, "0")}:${String(ss).padStart(2, "0")}`;
}

function formatCount(num) {
  if (isNaN(num) || num === 0) return "0";
  if (num < 1000) return String(num);
  if (num < 1000000) return (num / 1000).toFixed(1).replace(/\.0$/, "") + "K";
  return (num / 1000000).toFixed(1).replace(/\.0$/, "") + "M";
}

function hideOriginalList() {
  const originalList = document.querySelector(ORIGINAL_LIST_SELECTOR);
  if (originalList) originalList.style.display = "none";
  if (customListContainer) customListContainer.style.display = "block";
}

function showOriginalList() {
  const originalList = document.querySelector(ORIGINAL_LIST_SELECTOR);
  if (originalList) originalList.style.display = "block";
  if (customListContainer) customListContainer.style.display = "none";
}

// --- Main Render Function ---
function renderFullSortedList(episodes, sortType) {
  console.log(`Rendering UI for sort type: ${sortType}`);
  if (!customListContainer) return;

  // 1. Sort Locally
  let sortedList = [...episodes];
  if (sortType === "likes")
    sortedList.sort((a, b) => (b.like_count || 0) - (a.like_count || 0));
  else if (sortType === "plays")
    sortedList.sort((a, b) => (b.play_count || 0) - (a.play_count || 0));
  else if (sortType === "comments")
    sortedList.sort((a, b) => (b.comment_count || 0) - (a.comment_count || 0));
  else if (sortType === "oldest")
    sortedList.sort(
      (a, b) => new Date(a.release_date || 0) - new Date(b.release_date || 0)
    );

  let html = "<div>";

  sortedList.forEach((episode) => {
    const episodeUrl = `https://castbox.fm/episode/id${episode.cid}-id${episode.eid}`;
    // Use small cover for list to save bandwidth, fallback to big cover
    const coverUrl = episode.small_cover_url || episode.cover_url || "";
    const releaseDate = (episode.release_date || "1970-01-01T00:00:00Z").split(
      "T"
    )[0];

    // Logic for classes
    const isLiked = episode.is_liked ? true : false;
    // "btn-item like" is the class for empty heart, adding "liked" makes it filled/colored (handled by site CSS)
    const likeBtnClass = isLiked ? "btn-item like liked" : "btn-item like";

    // Handle Explicit Tag
    const explicitClass = episode.is_explicit ? "isExplicit" : "";

    // Description Cleanup (Remove HTML tags for the hover tooltip)
    let descText = episode.description || "";
    descText = descText.replace(/<[^>]*>?/gm, "");
    if (descText.length > 300) descText = descText.substring(0, 300) + "...";

    // --- HTML STRUCTURE ---
    // We use the exact structure from the analyzed HTML file.
    html += `
      <section class="episodeRow opacityinAnimate ${explicitClass}">
        <div class="ep-item">
          
          <div class="ep-item-cover">
            <a href="${episodeUrl}" target="_blank">
              <div class="coverImgContainer" style="display: inline-block; height: 100%; width: 100%; overflow: hidden; border-radius: 4px; background-color: inherit;">
                <img src="${coverUrl}" class="image" alt="${
      episode.title
    }" style="display: inline-block; opacity: 1; width: 100%; height: 100%;">
              </div>
            </a>
          </div>

          <div class="ep-item-con">
            <a href="${episodeUrl}" target="_blank">
              <p title="${episode.title}" class="ep-item-con-title">
                <span class="ellipsis" style="display: inline-block;">${
                  episode.title || "(Untitled)"
                }</span>
                </p>
            </a>
            
            <p class="ep-item-con-des">
              <span class="item icon date">${releaseDate}</span>
              <span class="item icon time">${formatDuration(
                episode.duration
              )}</span>
              
              <span class="item custom-stat-item" title="Likes">${statLikeIcon} ${formatCount(
      episode.like_count
    )}</span>
              <span class="item custom-stat-item" title="Plays">${statPlayIcon} ${formatCount(
      episode.play_count
    )}</span>
              <span class="item custom-stat-item" title="Comments">${statCommentIcon} ${formatCount(
      episode.comment_count
    )}</span>
            </p>

            <div class="ep-item-con-btns">
                <p class="${likeBtnClass} custom-like-btn" 
                   data-eid="${episode.eid}" 
                   data-cid="${episode.cid}" 
                   data-liked="${isLiked}"></p>
                
                <span class="btn-item add-2epl">
                    <div class="playlists-modal">
                        <div class="playlists-modal-target">
                            <div class="add-2epl-btn"></div>
                        </div>
                    </div>
                </span>
            </div>
          </div>

          <div class="ep-item-ctrls">
            <a class="ctrlItem play" href="${episodeUrl}" target="_blank"></a>
          </div>

        </div>
        
        <div class="audiobox"></div>
        
        <div class="ep-item-desmodal">
            <div class="ep-item-desmodal-con">${descText}</div>
        </div>
      </section>
    `;
  });

  html += `</div>`;
  customListContainer.innerHTML = html;

  hideOriginalList();

  // Attach Click Listeners (Like button)
  if (typeof attachLikeListeners === "function") {
    attachLikeListeners();
  }

  // Attach Hover Listeners (For Description)
  attachHoverListeners();

  console.log("Full custom list rendered with Perfect UI.");
}

// --- Hover Listener Logic (To mimic Castbox behavior) ---
function attachHoverListeners() {
  const rows = customListContainer.querySelectorAll(".episodeRow");

  rows.forEach((row) => {
    const cover = row.querySelector(".ep-item-cover");
    if (cover) {
      // When mouse enters the cover, add 'onImgFocus' to the main row
      cover.addEventListener("mouseenter", () => {
        row.classList.add("onImgFocus");
      });

      // When mouse leaves the cover, remove it
      cover.addEventListener("mouseleave", () => {
        row.classList.remove("onImgFocus");
      });
    }
  });
}

// --- Injection Function ---
function injectControls() {
  if (!window.location.href.includes("-id")) return;
  const targetContainer = document.querySelector(INJECTION_POINT_SELECTOR);
  const alreadyExists = document.getElementById("sorter-container-wrapper");

  if (targetContainer && !alreadyExists) {
    console.log("Injecting UI...");
    // ... (Rest of the dropdown injection code remains the same)
    const mainWrapper = document.createElement("div");
    mainWrapper.id = "sorter-container-wrapper";
    mainWrapper.className = "sorter-main-wrapper";

    const sorterComponent = document.createElement("div");
    sorterComponent.className = "sorter-component-container";

    const sorterControl = document.createElement("div");
    sorterControl.className = "sorter-control";
    sorterControl.addEventListener("click", toggleDropdown);

    const valueContainer = document.createElement("div");
    valueContainer.className = "sorter-value-container";
    customDropdownValue = document.createElement("div");
    customDropdownValue.className = "sorter-value-text";
    customDropdownValue.innerText = SORT_OPTIONS[currentSortType];
    valueContainer.appendChild(customDropdownValue);

    const indicators = document.createElement("div");
    indicators.className = "sorter-indicators";
    const arrow = document.createElement("div");
    arrow.className = "sorter-arrow-container";
    arrow.innerHTML =
      '<svg height="20" width="20" viewBox="0 0 20 20" aria-hidden="true" focusable="false"><path d="M4.516 7.548c0.436-0.446 1.043-0.481 1.576 0l3.908 3.747 3.908-3.747c0.533-0.481 1.141-0.446 1.574 0 0.436 0.445 0.408 1.197 0 1.615-0.406 0.418-4.695 4.502-4.695 4.502-0.217 0.223-0.502 0.335-0.787 0.335s-0.57-0.112-0.789-0.335c0 0-4.287-4.084-4.695-4.502s-0.436-1.17 0-1.615z" fill="currentColor"></path></svg>';
    indicators.appendChild(document.createElement("span")).className =
      "sorter-separator";
    indicators.appendChild(arrow);
    sorterControl.appendChild(valueContainer);
    sorterControl.appendChild(indicators);

    customDropdownMenu = document.createElement("div");
    customDropdownMenu.className = "sorter-menu";
    const menuList = document.createElement("div");
    menuList.className = "sorter-menu-list";

    Object.keys(SORT_OPTIONS).forEach((key) => {
      const option = document.createElement("div");
      option.className = "sorter-option";
      if (key === currentSortType) option.classList.add("is-selected");
      option.innerText = SORT_OPTIONS[key];
      option.dataset.value = key;
      option.addEventListener("click", handleOptionClick);
      menuList.appendChild(option);
    });

    customDropdownMenu.appendChild(menuList);
    sorterComponent.appendChild(sorterControl);
    sorterComponent.appendChild(customDropdownMenu);

    const statusContainer = document.createElement("div");
    statusContainer.id = "sorter-status-container";
    const statusText = document.createElement("span");
    statusText.id = "sorter-status-text";
    const loaderDiv = document.createElement("div");
    loaderDiv.id = "sorter-loader";
    loaderDiv.style.display = "none";
    loaderDiv.innerHTML = `<svg viewBox="0 0 32 32" width="32" height="32" fill="#ccc"><circle transform="translate(8)" cy="16" r="0"><animate attributeName="r" values="0; 4; 0; 0" dur="1.2s" repeatCount="indefinite" begin="0"></animate></circle><circle transform="translate(16)" cy="16" r="0"><animate attributeName="r" values="0; 4; 0; 0" dur="1.2s" repeatCount="indefinite" begin=".3"></animate></circle><circle transform="translate(24)" cy="16" r="0"><animate attributeName="r" values="0; 4; 0; 0" dur="1.2s" repeatCount="indefinite" begin=".6"></animate></circle></svg>`;
    statusContainer.appendChild(statusText);
    statusContainer.appendChild(loaderDiv);

    mainWrapper.appendChild(sorterComponent);
    mainWrapper.appendChild(statusContainer);
    targetContainer.prepend(mainWrapper);

    customListContainer = document.createElement("div");
    customListContainer.id = "custom-sorted-list-container";
    customListContainer.className = "trackListCon_list";
    customListContainer.style.display = "none";

    const originalList = document.querySelector(ORIGINAL_LIST_SELECTOR);
    if (originalList) originalList.after(customListContainer);
    else mainWrapper.after(customListContainer);
  }
}
